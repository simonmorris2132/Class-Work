...
/* let employeeCount = 0
for(let i = 0; i < employees.length; i++){
    let employeeID = employees[i][0] // 10001
    for(let j = 0; j < employeeDepartment.length; j++){
        let departmentID = employeeDepartment[j][1] // 'd006'
        for(let k = 0; k < departments.length; k++){
            if(employeeDepartment[j][0] === employeeID && departments[k][0] === departmentID){
                employeeCount++
                if(employeeCount > 1){
                    console.log(`${employeeCount} work in the ${departments[k][1]} department.`)
                } else if (employeeCount < 2) {
                    console.log(`${employeeCount} works in the ${departments[k][1]} department.`)
                }
            }
        }
    }
