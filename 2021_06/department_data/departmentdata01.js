...
//employee department array = [10001, "d005", "1986-06-26", "1999-01-01"],

//employee array = [10001,"1953-09-02","Georgi","Facello","M","1986-06-26"],

//salary array = [10001,60117,"1986-06-26","1987-06-26"],

//index 0 is the employee id#
//index 1 is the department id#
//index 2 is start date
//index 3 is end date
//swapped departments are lines 26+27, 35+36, and 47+48
//The employeeSalaries array has repeating numbers that basically show the update of income, and each line with the same number is an update to how much they gain each year until their release.

/* Challenge 1:
A. console log the first 10 elements from each array of the 4 arrays you just created
B. Create a legend for your data. This will help you know how to access the data you want without having to look back and forth between your code and the original data.
C) Log employee 1-40’s name and their department name
 */
...
for (let i = 0; i < employees.length; i++) {
  ...
  employName = employees[i][2] + " " + employees[i][3]
  
  /* if (employeeDepartment[i+1] != undefined && employeeID != employeeDepartment[i+1][0] || i == employeeDepartment.length -1) {
    const empIndex = employeeID - 10001
    const empDep = employeeDepartment[3]
...
Output the first 35 employee data.
    
B. Only log an employee name once, their salary should be their most recent wage (highest pay)
*/

let depos = [departments, employees, employeeDepartment, employeeSalaries]
let salary = []
let ID = []
let largestSalaries = []
...
for (let i = 0; i < salary.length; i++) {
  for (let j = 0; j < employees.length; j++) {
    if (employees[j][0] == salary[i][0]) {
      salary.push(employees[j][1],employees[j][2])
    }
  }
...
for (let i = 0; i < employeeDepartment.length; i++) {
  if (employeeDepartment[i][3] !== "9999-01-01") {
    console.log(employeeDepartment[i][3])
  }
}
